...
library(limma)
design=model.matrix(~factor(group_list,levels = c("primary breast tumor","brain metastasis")))
...
#hgu133a
if(!require(u133x3p.db))BiocManager::install("u133x3p.db")
library(u133x3p.db)
...
p <- ggplot(data = deg, 
  ...
  theme_bw()
for_label <- deg %>% 
  filter(abs(logFC) >2.5& P.Value< 0.01)
p +
