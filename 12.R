...
group_list = rep(c("G0","G1A"),times = 42)
group_list=factor(group_list,levels = c("G0","G1A"),ordered = T)
pairinfo = factor(rep(1: 42,each = 2))
exp = log2(exp+1)
dim(exp)
#PCA
{
...
#illuminaHumanv4
if(!require(illuminaHumanv4.db))BiocManager::install("illuminaHumanv4.db")
...
p <- ggplot(data = deg, 
  ...
  theme_bw()
for_label <- deg %>% 
  filter(abs(logFC) >1 & P.Value< 0.01)
p +
...
library(ggplot2)
deg$probe_id[1]
ggplot(data, aes(group,ILMN_1775073,fill=group)) +
  geom_boxplot() +
